...
//       - 'redirect'    (Gusto)     → /gusto-login (OAuth mock)
//       - 'credentials' / 'sftp'    → /credentials?provider={slug} (credential card)
//   • "Someone on my team…"  (delegate) → /invite?provider={slug}  (Stage 3)
//
// Credential entry no longer happens here: the old in-place Dialog modal moved
// to the dedicated /credentials card (Phase 08, CRED-01) so that /connecting is
// only ever reached AFTER credentials are submitted. This screen therefore
// never routes to /connecting itself.
//
