...
**Last updated:** 2026-06-09 — Added a scalable superscript ™ to FetchLogo (top-right of the wordmark; `fontSize: size * 0.35`, color text.primary; new `trademark` prop default true → `<FetchLogo trademark={false} />` hides it per instance). PNG untouched; single-component change; commit 603d4ee. Then — fixed the DELEGATE path: the invited recipient (`/recipient`) now goes through the SAME per-type credential entry as the self path (Gusto → /gusto-login; Principal → /credentials creds → /verify 2FA; SFTP → /credentials host+creds) via a new shared `src/lib/connectRoute.ts` `connectNowPath()` helper used by both /connect-method "I'll connect it now" and /recipient "Get Started" — no more hardcoded /connecting self-tail or &2fa=1 on the recipient path. `?provider=` already propagated through /invite → /invitation-sent → /recipient. DELG-01 done; commit 00b4129. Then — reverted the searchable Autocomplete on `/select-provider` back to the DS `Input select` dropdown (per request). (Superseded: Made the "Connection" dropdown a searchable combobox (MUI `Autocomplete`, no new dep): closed field unchanged (white/bordered/radius.md/notched "Connection"/chevron-down, navy `secondary.main` 2px focus); typing filters Gusto/Principal/SFTP with a "No connections found" empty state; select → Continue → /connect-method?provider=. Single-file change (select-provider). Commit 7164193. Then — reordered the self-path so 2FA precedes the "Establishing connection…" bridge (Principal: /credentials → /verify (2FA) → /connecting → /success). `/connecting` is now the FINAL step before /success for every path (the ?2fa=1 gate removed; /verify "Verify" → /connecting; recipient → /connecting without 2FA). Fixes the illogical "establishing before 2FA" order. Then — **Phase 08 (Credential Card) COMPLETE** on branch `demo-home`: replaced the plain MUI Dialog credential modal (Principal/SFTP self path) with a flow CARD `/credentials?provider=` that looks exactly like `/verify` (Principal: Email+Password "Sign in to Principal"; SFTP: Host+Username+Password "Connect via SFTP" per 8:365; Continue + Back). Reordered: `/connect-method` "I'll connect it now" → /credentials → Continue → /connecting → (Principal: /verify 2FA) → /success — so `/connecting` ("Establishing…") is NEVER shown before credentials. Gusto unchanged (→ /gusto-login). Modal + its state removed from /connect-method. CRED-01 done; commit 3fba51e. Gates tsc/lint/lint:tokens/build PASS; all routes 200 on :3001. Spec: `.planning/phases/08-credential-card/CRED-CARD-SPEC.md`.

Earlier — **Phase 07 (Connection Re-align) COMPLETE** on branch `demo-home`: all 10 connection screens re-aligned to the current Figma "Connection flows" (477:453) — /welcome "payroll data", /permissions 640+check-circle, /select-provider DS-Input "Connection" dropdown (replaced tonal Select), /verify purple active OTP cell, /success green check-circle, /invite Back+Send row, /invitation-sent no chip/mark, /recipient no caption; home copy "payroll system". DS Input gained a `select` mode; PermissionItem icon → check-circle. Per-type self routing PRESERVED (connect-method unchanged). REALIGN-01/02/03 done; commits f82caf3, c431b25, a93adfe. All routes 200 on :3001. Spec: `.planning/phases/07-connection-realign/REALIGN-SPEC.md`.

Earlier — **Phase 06 (Demo Home) COMPLETE** on branch `demo-home`: demo launcher at `/` (3 OptionRow flows), bespoke `/gusto-login` 2-step mock (sign-in → authorize → back to Fetch; Gusto hex scoped + excluded from lint:tokens), and connection-type realism (Gusto redirect → /gusto-login; SFTP modal per Figma 8:365; Principal creds→2FA). DEMO-01/02/03 done. Commits ef8b114, 1b94009, 1821a96. All routes 200 on :3001. Spec: `.planning/phases/06-demo-home/DEMO-HOME-SPEC.md`. (Prior on this branch: Phase 05 auth flows + logo/input/splash tweaks.)
